 ...
 */
public class CdbLuMapFastDomParser extends DefaultHandler {

    public HashMap<String,ArrayList<CdbLuForm>> lemmaMapCdbLuFormListMap;
    public HashMap<String,ArrayList<CdbLuForm>> luIdMapCdbLuFormList;

    public CdbLuMapFastDomParser() {
      initParser();
  }

  public void initParser () {
          lemmaMapCdbLuFormListMap = new HashMap<String,ArrayList<CdbLuForm>>();
          luIdMapCdbLuFormList = new HashMap<String,ArrayList<CdbLuForm>>();
  }

    static public void main (String[] args) {
        String luFilePath = args[0];
        CdbLuMapFastDomParser parser = new CdbLuMapFastDomParser();
        parser.readLuFile(luFilePath);
        System.out.println("parser.lemmaMapCdbLuFormListMap.size() = " + parser.lemmaMapCdbLuFormListMap.size());
    }
                        ...
                        try {
                            ...
                            if (doc!=null) {
                                ...
                                if (node.getNodeName().equals("cdb_lu")) {
                                    ...
                                    if (att!=null) {
                                        seq_nr = att.getNodeValue();
                                    }
                                    Node formNode= getSubNode(node, "form");
                                    if (formNode!=null) {
                                        attributes = formNode.getAttributes();
                                        if (attributes!=null) {
                                            CdbLuForm luForm = new CdbLuForm();
                                            luForm.setSeqNr(seq_nr);
                                            luForm.setLuId(c_lu_id);
                                            att = attributes.getNamedItem("form-spelling");
                                            if (att!=null) {
                                                luForm.setFormSpelling(att.getNodeValue());
                                            }
                                            att = attributes.getNamedItem("form-cat");
                                            if (att!=null) {
                                                luForm.setFormCat(att.getNodeValue());
                                            }

                                            if (lemmaMapCdbLuFormListMap.containsKey(luForm.getFormSpelling())) {
                                                ArrayList<CdbLuForm> lus = lemmaMapCdbLuFormListMap.get(luForm.getFormSpelling());
                                                lus.add(luForm);
                                             //   System.out.println("ids.toString() = " + ids.toString());
                                                lemmaMapCdbLuFormListMap.put(luForm.getFormSpelling(), lus);
                                            }
                                            else {
                                                ArrayList<CdbLuForm> lus = new ArrayList<CdbLuForm>();
                                                lus.add(luForm);
                                                lemmaMapCdbLuFormListMap.put(luForm.getFormSpelling(), lus);
                                            }
                                            if (luIdMapCdbLuFormList.containsKey(c_lu_id)) {
                                                ArrayList<CdbLuForm> lus = luIdMapCdbLuFormList.get(c_lu_id);
                                                lus.add(luForm);
                                                //   System.out.println("ids.toString() = " + ids.toString());
                                                luIdMapCdbLuFormList.put(c_lu_id, lus);
                                            }
                                            else {
                                                ArrayList<CdbLuForm> lus = new ArrayList<CdbLuForm>();
                                                lus.add(luForm);
